Simple increment/decrement contract
  ...
  (import "env" "is_left" (func $is_left (param i64) (result i32)))
  ...
  (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $push (call $z_add (call $pop) (call $pop)))) (else (call $push (call $get_right (local.get $1))) (call $swap)
...
FA2 with only transfer semantics
  ...
  (call $is_none (local.tee $1 (call $pop))) (if (then (call $dup (i32.const 8))) (else (call $push (call $get_some (local.get $1))) ))
  (call $push (call $cdr (call $pop)))
  (call $dup (i32.const 2))
  ...
  (call $push (call $update (call $pop) (call $pop) (call $pop)))
  (call $push (call $pair (call $pop) (call $pop))))
  (func $lambda.1 (local $1 i64) (call $push (call $cdr (local.tee $1 (call $pop)))) (call $push (call $car (local.get $1)))
  (call $swap)
  (call $push (call $cdr (local.tee $1 (call $pop)))) (call $push (call $car (local.get $1)))
  (call $push (call $map_get (call $pop) (call $pop)))
  (call $is_none (local.tee $1 (call $pop))) (if (then ) (else (call $push (call $get_some (local.get $1))) (call $swap)
  ...
  (call $dig (i32.const 2))
  (call $push (call $cdr (local.tee $1 (call $pop)))) (call $push (call $car (local.get $1)))
  (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $drop (i32.const 4))
  (call $push (call $const (i32.const 1))) (; "FA2_NOT_SUPPORTED" ;)
  (call $failwith (call $pop)) unreachable) (else (call $push (call $get_right (local.get $1))) (call $push (call $iter (call $pop) (table.get $closures (i32.const 2) (; $iter_lambda.2 ;) )))
  (call $swap)
  (call $dig (i32.const 2))
  (call $drop (i32.const 2))
  (call $push (call $nil))
  (call $push (call $pair (call $pop) (call $pop)))))) (else (call $push (call $get_right (local.get $1))) (call $drop (i32.const 4))
...
Quipuswap FA2 contract
  ...
  (call $swap)
  (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $dig (i32.const 2))
  ...
  (call $swap)
  (call $push (call $map_get (call $pop) (call $pop)))
  (call $is_none (local.tee $1 (call $pop))) (if (then (call $swap)
  (call $drop (i32.const 1))
  (call $swap)
  (call $drop (i32.const 1))
  (call $push (call $const (i32.const 1))) (; "Dex/function-not-set" ;)
  (call $failwith (call $pop)) unreachable) (else (call $push (call $get_some (local.get $1))) (call $dig (i32.const 3))
  (call $dig (i32.const 2))
  (call $dup (i32.const 1))
  ...
  (call $swap)
  (call $push (call $car (call $pop)))
  (call $push (call $pair (call $pop) (call $pop)))) (else (call $push (call $get_right (local.get $1))) (call $drop (i32.const 2))
  (call $dup (i32.const 1))
  (call $push (call $car (call $pop)))
  (call $push (call $car (call $pop)))
  (call $push (call $const (i32.const 2))) (; 8 ;)
  (call $push (call $map_get (call $pop) (call $pop)))
  (call $is_none (local.tee $1 (call $pop))) (if (then (call $push (call $const (i32.const 3))) (; "Dex/function-not-set" ;)
  (call $failwith (call $pop)) unreachable) (else (call $push (call $get_some (local.get $1))) (call $push (call $self))
  (call $push (call $address (call $pop)))
  (call $dig (i32.const 2))
  (call $dup (i32.const 1))
  (call $dug (i32.const 3))
  ...
  (call $swap)
  (call $push (call $car (call $pop)))
  (call $push (call $pair (call $pop) (call $pop)))))) (else (call $push (call $get_right (local.get $1))) (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $swap)
  (call $drop (i32.const 1))
  (call $swap)
  (call $dup (i32.const 1))
  (call $dug (i32.const 2))
  ...
  (call $push (call $pair (call $pop) (call $pop)))) (else (call $push (call $get_right (local.get $1))) (call $dig (i32.const 2))
  ...
  (call $swap)
  (call $push (call $car (call $pop)))
  (call $push (call $pair (call $pop) (call $pop)))))))) (else (call $push (call $get_right (local.get $1))) (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $dig (i32.const 2))
  ...
  (call $dup (i32.const 1))
  (call $dug (i32.const 2))
  (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $drop (i32.const 1))
  (call $push (call $const (i32.const 10))) (; 5 ;)) (else (call $push (call $get_right (local.get $1))) (call $drop (i32.const 1))
  (call $push (call $const (i32.const 11))) (; 0 ;)))) (else (call $push (call $get_right (local.get $1))) (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $drop (i32.const 1))
  (call $push (call $const (i32.const 12))) (; 4 ;)) (else (call $push (call $get_right (local.get $1))) (call $drop (i32.const 1))
  (call $push (call $const (i32.const 13))) (; 1 ;)))))) (else (call $push (call $get_right (local.get $1))) (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $drop (i32.const 1))
  (call $push (call $const (i32.const 14))) (; 2 ;)) (else (call $push (call $get_right (local.get $1))) (call $drop (i32.const 1))
  (call $push (call $const (i32.const 15))) (; 7 ;)))) (else (call $push (call $get_right (local.get $1))) (call $is_left (local.tee $1 (call $pop))) (if (then (call $push (call $get_left (local.get $1))) (call $drop (i32.const 1))
  (call $push (call $const (i32.const 16))) (; 6 ;)) (else (call $push (call $get_right (local.get $1))) (call $drop (i32.const 1))
  (call $push (call $const (i32.const 17))) (; 3 ;)))))))
  (call $push (call $map_get (call $pop) (call $pop)))
  (call $is_none (local.tee $1 (call $pop))) (if (then (call $drop (i32.const 2))
  (call $push (call $const (i32.const 18))) (; "Dex/function-not-set" ;)
  (call $failwith (call $pop)) unreachable) (else (call $push (call $get_some (local.get $1))) (call $dig (i32.const 2))
  (call $dig (i32.const 3))
  (call $dup (i32.const 1))
